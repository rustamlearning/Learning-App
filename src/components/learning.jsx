  ...
  return (
    <SectionCard>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.16em] text-[#0284c7]">Misi hari ini</p>
      ...
      <div className="divide-y divide-[#0B3A5B]/10">
        {missions.map((mission, index) => (
          <div key={mission} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
            <span className={`flex h-8 w-8 items-center justify-center rounded-[0.75rem] ${index < 2 ? 'bg-emerald-50 text-emerald-600' : 'bg-[#E0F2FE] text-[#0284c7]'}`}>
      ...
      <p className="glass-inset mt-4 rounded-[0.9rem] px-4 py-3 text-sm font-extrabold text-amber-700">Reward selesai: +50 XP</p>
    </SectionCard>
  )
}
      ...
      <div className="flex min-w-[620px] items-center gap-2">
        {steps.map((step, index) => (
          <div key={step} className="flex flex-1 items-center">
            <div className={`grid h-14 min-w-24 place-items-center rounded-[0.95rem] text-center text-xs font-extrabold ${index < 4 ? 'glass-dark text-white' : 'glass-inset text-[#0284c7]'}`}>
              {step}
            </div>
            {index < steps.length - 1 && <ArrowRight className="mx-2 h-5 w-5 flex-shrink-0 text-[#0284c7]/50" />}
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <SectionCard className="archipelago-soft island-wave">
      <div className="mb-4 flex items-center gap-3">
        <div className="flex h-12 w-12 items-center justify-center rounded-[0.9rem] bg-[#0B3A5B] text-white shadow-[0_12px_28px_rgba(15,31,42,0.14)]">
          <Sparkles size={22} />
        </div>
        <div>
          <p className="text-xs font-black uppercase tracking-[0.16em] text-[#0284c7]">IsleClub English Corner</p>
...
function Info({ label, value }) {
  return (
    <div className="glass-inset rounded-[0.95rem] p-4">
  ...
  return (
    <SectionCard className="min-h-[34rem]">
      ...
      <div className="mb-4 flex gap-2 overflow-x-auto pb-1">
        {chips.map((chip) => <button key={chip} onClick={() => submit(chip)} className="glass-inset flex-shrink-0 rounded-[0.8rem] px-3 py-2 text-xs font-bold text-[#0284c7]">{chip}</button>)}
      </div>
      <div className="glass-inset thin-scrollbar mb-4 max-h-80 space-y-3 overflow-y-auto rounded-[1rem] p-3">
        {messages.map((message, index) => (
          <div key={`${message.role}-${index}`} className={`max-w-[88%] rounded-[1rem] px-4 py-3 text-sm leading-6 whitespace-pre-wrap ${message.role === 'assistant' ? 'bg-white/58 text-gray-700 shadow-sm ring-1 ring-white/60 backdrop-blur-xl' : 'glass-dark ml-auto text-white'}`}>
            <FormattedAIText text={message.content} />
          </div>
        ))}
      </div>
      <form onSubmit={(event) => { event.preventDefault(); submit() }} className="flex gap-2">
        <input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Tulis pertanyaan belajarmu..." className="min-h-12 flex-1 rounded-[0.9rem] border border-white/70 bg-white/48 px-4 text-sm outline-none backdrop-blur-xl focus:border-[#0284C7]" />
        <button aria-label="Kirim pertanyaan" className="grid h-12 w-12 place-items-center rounded-[0.9rem] bg-[#0B3A5B] text-white shadow-[0_12px_28px_rgba(15,31,42,0.14)]"><Send size={18} /></button>
      </form>
    </SectionCard>
  )
}
        ...
        <div className="mt-5 grid gap-3">
          {[
            ...
            <label key={key} className="grid gap-1 text-sm font-bold text-gray-700">
              {label}
              <input value={options[key]} onChange={(event) => setOptions((current) => ({ ...current, [key]: event.target.value }))} placeholder={key === 'topic' ? 'Tulis topik yang ingin dibuat' : 'Isi sesuai kebutuhan'} className="rounded-[0.9rem] border border-white/70 bg-white/48 px-4 py-3 text-sm font-medium outline-none backdrop-blur-xl focus:border-[#0284C7]" />
            </label>
          ))}
          <QuickActionButton icon={Sparkles} label="Generate" onClick={generate} disabled={!options.topic.trim()} />
        </div>
      </SectionCard>
      <SectionCard className="glass-dark text-white">
        <div className="mb-4 flex items-center justify-between">
...
export function FlashcardDeck({ deck }) {
  ...
  return (
    <SectionCard>
      ...
      <button onClick={() => setFlipped(!flipped)} className="glass-dark min-h-44 w-full rounded-[1rem] p-5 text-left text-white">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <button className="glass-inset rounded-2xl px-4 py-3 text-sm font-bold text-emerald-700">Saya Tahu</button>
        <button className="glass-inset rounded-2xl px-4 py-3 text-sm font-bold text-amber-700"><RotateCcw size={15} className="mr-1 inline" /> Ulangi</button>
      </div>
    </SectionCard>
  )
}
