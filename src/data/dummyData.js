...
export const questions = Array.from({ length: 20 }).map((_, index) => ({
  id: `question-${index + 1}`,
  questionText: index % 2 ? 'Which sentence uses simple present tense correctly?' : 'What is the purpose of descriptive text?',
  options: ['To describe details', 'To argue an opinion', 'To tell steps', 'To announce news'],
  correctAnswer: 'To describe details',
  explanation: 'Descriptive text explains a person, place, animal, or object in detail.',
  subject: index % 2 ? 'Bahasa Inggris' : 'Matematika',
  className: classes[index % classes.length].name,
  topic: index % 2 ? 'Grammar' : 'Text Type',
  difficulty: ['Mudah', 'Sedang', 'Sulit'][index % 3],
  type: ['Pilihan ganda', 'Benar/salah', 'Isian', 'Essay'][index % 4],
}))
